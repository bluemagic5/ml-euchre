...
class Player(object):
    ...
    def getHandCsv(self):
        ...
        for i in range(len(self.hand)):
            if i == 0:
                handCsv = "%s" % (self.hand[i])
            else:
                handCsv = "%s,%s" % (handCsv, self.hand[i])
        return handCsv
    ...
    def setHand(self, hand):
        assert len(
            hand) == 5, "from setHand of %s, hand needs to be 5" % self.name
        self.hand = hand
...
class RandomPlay(Player):
    ...
    def pickSuit(self, out_suit, stuck):
        if stuck or random.choice([True, False]):
    ...
    def pickUp(self, top):
        assert type(
            top) == Card, "pickUp was given a %s, it wants a Card" % type(top)
...
class SimpleStat(Player):
    ...
    def updateInfo(self, winner):
        ...
        for c, p in game.center.items():
            ...
            # if they lost then we can consider that they do not have any
            # better cards in their hand
    ...
    def pickSuit(self, out_suit, stuck):
        suit_counts = { spade: 0, heart: 0, diamond: 0, club: 0 }
        del suit_counts[out_suit]

        highSuit = None
        highVal = 0
        for s in suit_counts:
            for c in self.hand:
                val = hypo_curCardVal(c, s)
                suit_counts[s] += val
            if suit_counts[s] > highVal:
                highSuit = s
                highVal = suit_counts[s]

        # If the player is stuck or has a good hand call the suit
        # * For reference having both bowers and the ace (which is a guaranteed
        #   point) is 75 points
        if stuck or highVal >= 90:
            return highSuit

        return None

    def pickUp(self, card):
        nums = { spade: 0, heart: 0, diamond: 0, club: 0 }
        ...
        if len(d_cards) == 1:
            ...
            try:
                ...
                # if that doesnt work then it doesn't matter because you have
                # all the trump
...
class RealPlayer(Player):
    ...
    def pickSuit(self, out_suit, stuck):
        ...
        elif query_yes_no("Do you want to pick a suit? (out suit is %s)" % out_suit):
            print "Pick %s, %s, or %s." % (open_suits[0], open_suits[1], open_suits[2])

            while True:
                choice = raw_input(
                    "Enter the first letter of the suit name: ").lower()
                if choice[0] == 'd':
                    if diamond in open_suits:
                        return diamond
                elif choice[0] == 'h':
    ...
    def pickUp(self, center_card):
        ...
        move = int(
            raw_input("Please enter the # of the card you wish to discard: "))
        while True:
